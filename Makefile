...
$(EXEC): main.cpp node.o graph.o
	$(CC) $(CFLAGS) main.cpp node.o graph.o -o $(EXEC)

node.o: headers/node.cpp # esse eh um exemplo, nao utilize esse tipo de nome (foo.cc, foo.h, foo.o) para seus arquivos
	$(CC) $(CFLAGS) -c headers/node.cpp -o node.o

graph.o: headers/graph.cpp # esse eh um exemplo, nao utilize esse tipo de nome (foo.cc, foo.h, foo.o) para seus arquivos
	$(CC) $(CFLAGS) -c headers/graph.cpp -o graph.o

test: $(EXEC)
	@bash run_tests.sh $(EXEC) $(TMPOUT)

clean: # remove todos os arquivos temporarios que forem gerados pela compilacao
	rm -rf graph.o node.o
